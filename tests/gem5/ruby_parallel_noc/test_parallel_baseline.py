...
gem5_verify_config(
    name="ruby-parallel-noc-parallel-baseline",
    fixtures=(),
    verifiers=(
        NamedMatchRegex(
            "parallel-noc-mode-line",
            r"^PARALLEL_NOC_MODE=parallel REQUESTED=parallel "
            r"WORKERS=3 NUM_CPUS=4$",
        ),
        NamedMatchRegex(
            "parallel-noc-coordinator-line",
            r"^PARALLEL_NOC_COORDINATOR mode=parallel workers=3$",
        ),
        NamedMatchRegex(
        ...
        NamedMatchRegex(
            ...
            r"^Exiting @ tick 2001 because Network Tester completed "
            r"simCycles$",
        ),
        NamedMatchRegex(
