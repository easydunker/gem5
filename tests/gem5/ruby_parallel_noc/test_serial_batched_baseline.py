...
gem5_verify_config(
    name="ruby-parallel-noc-serial-batched-baseline",
    fixtures=(),
    verifiers=(
        NamedMatchRegex(
            "parallel-noc-mode-line",
            r"^PARALLEL_NOC_MODE=serial_batched REQUESTED=serial_batched "
            r"WORKERS=2 NUM_CPUS=4$",
        ),
        NamedMatchRegex(
            "parallel-noc-coordinator-line",
            r"^PARALLEL_NOC_COORDINATOR mode=serial_batched workers=2$",
        ),
        NamedMatchRegex(
